...
def main(): #메소드 생성자
    ...
    ## 3. 교시 포인트 설정 ######################
    p1 = data.get_position( "pos1", 6 ) #물건집는 위치1
    p2 = data.get_position( "pos1", 8 ) #물건집는 위치2
    p3 = data.get_position( "pos1", 7 ) #물건집는 위치3
    #p11 = p1.offset(dx=10,dy=20,dz=-10)
    #팔레트 정의
    pal = Pallet() 
    pal.init_3(p1,p2,p3,301,301)#팔레트를 정의했는데 3점교시를 하겠다. 팔레트 위치 크기정의 (5,4)

    #p01 = pal.get_pos(0,1) #설정후 잡을 위치설정
    #작업 위치 검색
    
    #p00 = pal.get_pos(0,0) #설정후 잡을 위치설정
    p01 = pal.get_pos(150,150)
    #p02 = pal.get_pos(0,2)
